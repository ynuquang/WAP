'''
Build a WAP model with DenseNet
'''
import theano
import theano.tensor as tensor
from theano.sandbox.rng_mrg import MRG_RandomStreams as RandomStreams
from theano.tensor.signal import pool
from theano.gpuarray.dnn import dnn_batch_normalization_train
from theano.gpuarray.dnn import dnn_batch_normalization_test
...
# conv layer
def param_init_DenseNet(rng, options, params, prefix='dense'):
    name = 'input'
    params[_p(prefix, name)] = conv_norm_weight(rng, options['input_channels'], options['channel_conv1'], options['kernel_conv1'])
    bn_gamma  = numpy.asarray(rng.uniform(low=-1.0/math.sqrt(options['channel_conv1']),high=1.0/math.sqrt(options['channel_conv1']),size=options['channel_conv1']),dtype=numpy.float32)
    params[_p(_p(prefix, name), 'bn_gamma')] = bn_gamma[None, :, None, None]
    bn_beta = numpy.zeros((options['channel_conv1'])).astype('float32')
    ...
    for block in range(len(options['DenseBlock'])):        
        for level in range(options['DenseBlock'][block]):
            if options['Bottleneck']:
                ...
                params[_p(prefix, name)] = conv_norm_weight(rng, dense_channels, 4*k, [1,1])
                bn_gamma  = numpy.asarray(rng.uniform(low=-1.0/math.sqrt(4*k),high=1.0/math.sqrt(4*k),size=4*k),dtype=numpy.float32)
                params[_p(_p(prefix, name), 'bn_gamma')] = bn_gamma[None, :, None, None]
                bn_beta = numpy.zeros((4*k)).astype('float32')
                params[_p(_p(prefix, name), 'bn_beta')] = bn_beta[None, :, None, None]
                name = 'b{}_l{}'.format(block, level)
                params[_p(prefix, name)] = conv_norm_weight(rng, 4*k, k, [3,3])
                bn_gamma  = numpy.asarray(rng.uniform(low=-1.0/math.sqrt(k),high=1.0/math.sqrt(k),size=k),dtype=numpy.float32)
                params[_p(_p(prefix, name), 'bn_gamma')] = bn_gamma[None, :, None, None]
                bn_beta = numpy.zeros((k)).astype('float32')
                params[_p(_p(prefix, name), 'bn_beta')] = bn_beta[None, :, None, None]
            else:
                name = 'b{}_l{}'.format(block, level)
                params[_p(prefix, name)] = conv_norm_weight(rng, dense_channels, k, [3,3])
                bn_gamma  = numpy.asarray(rng.uniform(low=-1.0/math.sqrt(k),high=1.0/math.sqrt(k),size=k),dtype=numpy.float32)
                params[_p(_p(prefix, name), 'bn_gamma')] = bn_gamma[None, :, None, None]
                bn_beta = numpy.zeros((k)).astype('float32')
                params[_p(_p(prefix, name), 'bn_beta')] = bn_beta[None, :, None, None]
        ...
        if block < len(options['DenseBlock']) - 1:
            ...
            params[_p(prefix, name)] = conv_norm_weight(rng, dense_channels, compress_channels, [1,1])
            bn_gamma  = numpy.asarray(rng.uniform(low=-1.0/math.sqrt(compress_channels),high=1.0/math.sqrt(compress_channels),size=compress_channels),dtype=numpy.float32)
            params[_p(_p(prefix, name), 'bn_gamma')] = bn_gamma[None, :, None, None]
            bn_beta = numpy.zeros((compress_channels)).astype('float32')
            params[_p(_p(prefix, name), 'bn_beta')] = bn_beta[None, :, None, None]
            dense_channels = compress_channels

    return params, dense_channels

def init_bn_params(options):
    params = OrderedDict()
    prefix='dense'
    name = 'input'
    bn_mean = numpy.zeros((options['channel_conv1'])).astype('float32')
    params[_p(_p(prefix, name), 'bn_mean')] = bn_mean[None, :, None, None]
    bn_var = numpy.ones((options['channel_conv1'])).astype('float32')
    ...
    for block in range(len(options['DenseBlock'])):        
        for level in range(options['DenseBlock'][block]):
            if options['Bottleneck']:
                name = 'B_b{}_l{}'.format(block, level)
                bn_mean = numpy.zeros((4*k)).astype('float32')
                params[_p(_p(prefix, name), 'bn_mean')] = bn_mean[None, :, None, None]
                bn_var = numpy.ones((4*k)).astype('float32')
                params[_p(_p(prefix, name), 'bn_var')] = bn_var[None, :, None, None]
                name = 'b{}_l{}'.format(block, level)
                bn_mean = numpy.zeros((k)).astype('float32')
                params[_p(_p(prefix, name), 'bn_mean')] = bn_mean[None, :, None, None]
        ...
        if block < len(options['DenseBlock']) - 1:
            compress_channels = int(dense_channels * options['Transition'])
            name = 'C_b{}'.format(block)
            bn_mean = numpy.zeros((compress_channels)).astype('float32')
            params[_p(_p(prefix, name), 'bn_mean')] = bn_mean[None, :, None, None]
            bn_var = numpy.ones((compress_channels)).astype('float32')
            params[_p(_p(prefix, name), 'bn_var')] = bn_var[None, :, None, None]
            dense_channels = compress_channels
...
def init_params(options):
    ...
    params, dense_channels = param_init_DenseNet(rng, options, params, prefix='dense')

    ctxdim = dense_channels
...
def build_model(tparams, bn_tparams, options):
    ...
    bn_var = bn_tparams[_p(_p('dense', name), 'bn_var')]
    oup, s_mean, s_invstd, opt_ret[_p(_p('dense', name), 'bn_mean')], opt_ret[_p(_p('dense', name), 'bn_var')] =\
    ...
    for block in range(len(options['DenseBlock'])):        
        ...
        if block < len(options['DenseBlock']) - 1:
            ...
            if options['use_dropout']:
                oup = dropout_layer(oup, 0.2, use_noise,trng)
            oup = pool.pool_2d(oup,(2,2),ignore_border=False,stride=(2,2),pad=(0,0),mode='average_exc_pad')
            dense_oup = oup
            x_mask = x_mask[:,0::2,0::2]
...
def build_sampler(tparams, bn_tparams, options, trng, use_noise):
    ...
    bn_mean = bn_tparams[_p(_p('dense', name), 'bn_mean')]
    bn_var = bn_tparams[_p(_p('dense', name), 'bn_var')]
    oup = dnn_batch_normalization_test(oup, bn_gamma, bn_beta, bn_mean, bn_var, mode='spatial')
    oup = tensor.nnet.relu(oup)
    oup = pool.pool_2d(oup,(2,2),ignore_border=False,stride=(2,2),pad=(0,0),mode='max')
    dense_oup = oup
...
def train(dim_word=100,  # word vector dimensionality
    ...
    # apply L2 regularization on DenseNet weights
    if decay_c > 0.:
    ...
    for eidx in range(max_epochs):
        ...
        for x, y in train:
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                ...
                pkl.dump(model_options, open('%s.pkl' % saveto, 'w'))
                print('Done')
